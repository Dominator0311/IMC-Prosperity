...
# ========================================================================
# ROUND-1 UPLOAD VARIANT: ash_deep_f2c
# Label   : Round-1 Phase-F F2c — weighted_mid ASH + buy_hold PEPPER
# Purpose : ASH deep-dive upload F2c. ASH uses weighted_mid (Phase-B/B1 winner, Phase-E stress-robust across all six synthetic tapes) at shipped edges (m=1.5, t=0.5). PEPPER pinned at buy-and-hold with max_aggressive_size=80 per the deep-dive plan's pin. Expected: beat shipped C_h1_alt by ~+200-1200 on official ASH PnL if the weighted_mid stress-robustness finding transfers.
# Factory : src.core.config.round1_ash_deep_f2c_engine_config
# Built   : 2026-04-16T12:44:28.399178+00:00 (commit 043faac)
# Embedded product configs:
#   ASH_COATED_OSMIUM: {'position_limit': 80, 'strategy_name': 'market_making', 'fair_value_method': 'weighted_mid', 'fair_value_fallbacks': ('wall_mid', 'mid'), 'anchor_price': 10000.0, 'taker_edge': 0.5, 'maker_edge': 1.5, 'quote_size': 5, 'max_aggressive_size': 10, 'inventory_skew': 4.0, 'flatten_threshold': 0.7, 'history_length': 48, 'taker_edge_buy': None, 'taker_edge_sell': None, 'early_window': 0, 'early_taker_edge_buy': None, 'early_taker_edge_sell': None, 'early_short_cap': None, 'early_short_skew_mult': 1.0, 'early_short_flatten': None}
#   INTARIAN_PEPPER_ROOT: {'position_limit': 80, 'strategy_name': 'buy_and_hold', 'fair_value_method': 'mid', 'fair_value_fallbacks': (), 'anchor_price': None, 'taker_edge': 1.0, 'maker_edge': 1.5, 'quote_size': 4, 'max_aggressive_size': 80, 'inventory_skew': 2.0, 'flatten_threshold': 0.8, 'history_length': 48, 'taker_edge_buy': None, 'taker_edge_sell': None, 'early_window': 0, 'early_taker_edge_buy': None, 'early_taker_edge_sell': None, 'early_short_cap': None, 'early_short_skew_mult': 1.0, 'early_short_flatten': None}
# ========================================================================
...
def round1_v2_clean_engine_config() -> EngineConfig:
    """V2_clean — opening-seeded core-long PEPPER + wall_mid ASH.

    See ``outputs/round_1/pepper_corelong_v2/final_recommendation.md``.
    Additive; PEPPER ``strategy_name="pepper_core_long"`` is wired by
    the V2_clean submission bundle, not the shipped ``STRATEGY_REGISTRY``.
    """
    return _round1_engine_with(
        ASH_COATED_OSMIUM=dict(
            fair_value_method="wall_mid",
...
def round1_v3_nearhold_engine_config() -> EngineConfig:
    """V3_nearhold — near-buy-and-hold PEPPER + wall_mid ASH.

    See ``outputs/round_1/pepper_sparse_overlay/final_recommendation.md``.
    Additive; PEPPER ``strategy_name="pepper_core_long"`` is wired by
    the V3_nearhold submission bundle, not the shipped ``STRATEGY_REGISTRY``.
    """
    return _round1_engine_with(
        ASH_COATED_OSMIUM=dict(
            fair_value_method="wall_mid",
...
# Phase-F ASH deep-dive variants. See outputs/round_1/ash_deep_dive/.
_PEP = dict(strategy_name="buy_and_hold", fair_value_method="mid",
            fair_value_fallbacks=(), max_aggressive_size=80)
_WALL = ("mid", "microprice")
_WMID = ("wall_mid", "mid")
_SPECS: dict[str, tuple[str, str, tuple[str, ...], float]] = {
    "f2a": ("ash_avellaneda_stoikov", "wall_mid", _WALL, 2.0),
    "f2b": ("ash_shape_override", "wall_mid", _WALL, 2.5),
    "f2c": ("market_making", "weighted_mid", _WMID, 1.5),
    "f2d": ("market_making", "wall_mid", _WALL, 2.5),
    "f3a": ("ash_shape_override", "weighted_mid", _WMID, 2.5),
    "f3b": ("ash_avellaneda_stoikov", "weighted_mid", _WMID, 2.0),
    "f3c": ("ash_cartea_skew", "wall_mid", _WALL, 2.5),
    "f3d": ("ash_avellaneda_stoikov", "wall_mid", _WALL, 2.0),
    "f3e": ("ash_cartea_skew", "wall_mid", _WALL, 2.0),
}


def _ash_deep_config(variant: str) -> EngineConfig:
    s, fv, fb, m = _SPECS[variant]
    return _round1_engine_with(
        ASH_COATED_OSMIUM=dict(strategy_name=s, fair_value_method=fv,
            fair_value_fallbacks=fb, maker_edge=m, taker_edge=0.5,
            inventory_skew=4.0, flatten_threshold=0.7, history_length=48),
        INTARIAN_PEPPER_ROOT=dict(_PEP),
    )


def round1_ash_deep_f2a_engine_config() -> EngineConfig: return _ash_deep_config("f2a")
def round1_ash_deep_f2b_engine_config() -> EngineConfig: return _ash_deep_config("f2b")
def round1_ash_deep_f2c_engine_config() -> EngineConfig: return _ash_deep_config("f2c")
def round1_ash_deep_f3b_engine_config() -> EngineConfig: return _ash_deep_config("f3b")
def round1_ash_deep_f3c_engine_config() -> EngineConfig: return _ash_deep_config("f3c")
def round1_ash_deep_f3d_engine_config() -> EngineConfig: return _ash_deep_config("f3d")
def round1_ash_deep_f3e_engine_config() -> EngineConfig: return _ash_deep_config("f3e")


def round1_ash_deep_f3a_engine_config() -> EngineConfig: return _ash_deep_config("f3a")
def round1_ash_deep_f2d_engine_config() -> EngineConfig: return _ash_deep_config("f2d")
...
class Trader:
    def __init__(
        ...
        *,
        ...
        self.config = config or round1_ash_deep_f2c_engine_config()
