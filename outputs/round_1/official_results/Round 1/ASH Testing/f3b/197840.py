...
# ========================================================================
# ROUND-1 UPLOAD VARIANT: ash_deep_f3b
# Label   : Round-1 Phase-F F3b — D7 weighted_mid + AS gamma=2e-6
# Purpose : Phase-D D7: weighted_mid FV (Phase-B winner) + AS quote formula (Phase-C winner). Tests whether stacking the two winning components compounds. Stress-tape robust (zero loss on all six Phase-E tapes).
# Factory : src.core.config.round1_ash_deep_f3b_engine_config
# Built   : 2026-04-16T12:46:05.478591+00:00 (commit 043faac)
# Embedded product configs:
#   ASH_COATED_OSMIUM: {'position_limit': 80, 'strategy_name': 'ash_avellaneda_stoikov', 'fair_value_method': 'weighted_mid', 'fair_value_fallbacks': ('wall_mid', 'mid'), 'anchor_price': 10000.0, 'taker_edge': 0.5, 'maker_edge': 2.0, 'quote_size': 5, 'max_aggressive_size': 10, 'inventory_skew': 4.0, 'flatten_threshold': 0.7, 'history_length': 48, 'taker_edge_buy': None, 'taker_edge_sell': None, 'early_window': 0, 'early_taker_edge_buy': None, 'early_taker_edge_sell': None, 'early_short_cap': None, 'early_short_skew_mult': 1.0, 'early_short_flatten': None}
...
class Trader:
    def __init__(
        ...
        *,
        ...
        self.config = config or round1_ash_deep_f3b_engine_config()
...
_ASH_DEEP_F3B_PARAMS = AvellanedaStoikovParams(
    gamma=2e-06,
    sigma=2.0,
    k=0.4,
    horizon=100000,
...
def _ash_deep_f3b_ash_strategy_factory(
    fve: FairValueEngine, sig: SignalEngine
) -> BaseStrategy:
    return AshAvellanedaStoikovStrategy(fve, sig, _ASH_DEEP_F3B_PARAMS)

KNOWN_STRATEGY_NAMES = tuple(
    sorted(set(KNOWN_STRATEGY_NAMES) | {"ash_avellaneda_stoikov"})
)
STRATEGY_REGISTRY = MappingProxyType(
    dict(STRATEGY_REGISTRY, ash_avellaneda_stoikov=_ash_deep_f3b_ash_strategy_factory)
)