...
# ========================================================================
# ROUND-1 UPLOAD VARIANT: ash_deep_j7
# Label   : Round-1 Phase-J J7 — weighted 3/1/1/1 + spread-gated
# Purpose : Phase-J post-I2a combo. Combines J2's inner-heavy weighting (3/1/1/1 → inner 50%, outers 17% each) with J3's spread-gating (outer levels only when spread>=16). Tests whether the two mechanisms compound. Local +3 581 (below J2 by 72 but the gating filters low-value outer-level ticks). Projected official ~+1 468 if transfer ratio benefits from the cleaner mechanism.
# Factory : src.core.config.round1_ash_deep_j7_engine_config
# Built   : 2026-04-16T17:47:01.506321+00:00 (commit 043faac)
...
# ASH strategy: AshLadderStrategy (research-only, inlined at bundle tail; NOT in shipped STRATEGY_REGISTRY)
# ASH params: {'edges': (2.5, 5.0, 8.0, 12.0), 'size_mults': (1.0, 1.5, 2.0, 3.0), 'skew_coef': 2.0, 'flatten_threshold': 0.7, 'weights': (3, 1, 1, 1), 'spread_gate_enabled': True, 'gate_spread': 16}
# ========================================================================
...
def round1_ash_deep_i2a_engine_config() -> EngineConfig:
    return _round1_engine_with(
        ASH_COATED_OSMIUM=dict(
...
def round1_ash_deep_j2_engine_config() -> EngineConfig:
    return _round1_ladder_base()

def round1_ash_deep_j3_engine_config() -> EngineConfig:
    return _round1_ladder_base()

def round1_ash_deep_j7_engine_config() -> EngineConfig:
    return _round1_ladder_base()
...
class Trader:
    def __init__(
        ...
        *,
        ...
        self.config = config or round1_ash_deep_j7_engine_config()
...
class AshLadderStrategy(BaseStrategy):
    ...
    def _pick_level(
        ...
        if params.weights is None:
            return counter % n
        schedule = getattr(self, "_cached_schedule", None)
        if schedule is None:
            schedule = []
            for i, w in enumerate(params.weights):
                schedule.extend([i] * int(w))
            if not schedule:
                schedule = list(range(n))
            object.__setattr__(self, "_cached_schedule", tuple(schedule))
    ...
    def generate_intent(self, context: StrategyContext) -> SignalIntent:
        ...
        bid_size = max(1, int(round(base_size * size_mult)))
        ask_size = max(1, int(round(base_size * size_mult)))
...
_ASH_DEEP_J7_PARAMS = LadderParams(
...
def _ash_deep_j7_ash_strategy_factory(
    fve: FairValueEngine, sig: SignalEngine
) -> BaseStrategy:
    return AshLadderStrategy(fve, sig, _ASH_DEEP_J7_PARAMS)

KNOWN_STRATEGY_NAMES = tuple(
    sorted(set(KNOWN_STRATEGY_NAMES) | {"ash_ladder"})
)
STRATEGY_REGISTRY = MappingProxyType(
    dict(STRATEGY_REGISTRY, ash_ladder=_ash_deep_j7_ash_strategy_factory)
)