...
# ========================================================================
# ROUND-1 UPLOAD VARIANT: ash_deep_j3
# Label   : Round-1 Phase-J J3 — spread-gated 4-level ladder
# Purpose : Phase-J post-I2a test. 4-level ladder (2.5/5/8/12) but outer levels (index>0) only fire when book_spread >= 16. When spread is tight (<16), forces inner-level quoting. Theory: outer-level quotes at edge=12 when spread=10 are meaningless (they sit 7 ticks beyond the book). Gating prevents wasted outer-level ticks. Local +3 631 (vs I2a +3 666). Projected official ~+1 452.
# Factory : src.core.config.round1_ash_deep_j3_engine_config
# Built   : 2026-04-16T17:47:01.373635+00:00 (commit 043faac)
...
# ASH params: {'edges': (2.5, 5.0, 8.0, 12.0), 'size_mults': (1.0, 1.5, 2.0, 3.0), 'skew_coef': 2.0, 'flatten_threshold': 0.7, 'spread_gate_enabled': True, 'gate_spread': 16}
# ========================================================================
...
class Trader:
    def __init__(
        ...
        *,
        ...
        self.config = config or round1_ash_deep_j3_engine_config()
...
_ASH_DEEP_J3_PARAMS = LadderParams(
...
def _ash_deep_j3_ash_strategy_factory(
    fve: FairValueEngine, sig: SignalEngine
) -> BaseStrategy:
    return AshLadderStrategy(fve, sig, _ASH_DEEP_J3_PARAMS)

KNOWN_STRATEGY_NAMES = tuple(
    sorted(set(KNOWN_STRATEGY_NAMES) | {"ash_ladder"})
)
STRATEGY_REGISTRY = MappingProxyType(
    dict(STRATEGY_REGISTRY, ash_ladder=_ash_deep_j3_ash_strategy_factory)
)