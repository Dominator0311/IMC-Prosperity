...
# ========================================================================
# ROUND-1 UPLOAD VARIANT: f5
# Label   : Round-1 Phase-9 asymmetric-taker PEPPER + alt ASH
# Purpose : Phase-9 fastsearch winner. Wall-based ASH (same leg as H1 / alt) + promoted PEPPER with per-side taker edges (buy=1.5, sell=3.0). A surgical sell-widening that keeps promoted's inventory profile and targets the official-day Baseline failure mode (early wrong-side PEPPER sells) without adopting alt PEPPER's near-limit tail.
# Factory : src.core.config.round1_f5_engine_config
# Built   : 2026-04-15T13:04:21.965839+00:00 (commit 51efc59)
# Embedded product configs:
#   ASH_COATED_OSMIUM: {'position_limit': 50, 'fair_value_method': 'wall_mid', 'fair_value_fallbacks': ('mid', 'microprice'), 'anchor_price': 10000.0, 'taker_edge': 0.5, 'maker_edge': 1.5, 'quote_size': 5, 'max_aggressive_size': 10, 'inventory_skew': 4.0, 'flatten_threshold': 0.7, 'history_length': 48, 'taker_edge_buy': None, 'taker_edge_sell': None, 'early_window': 0, 'early_taker_edge_buy': None, 'early_taker_edge_sell': None, 'early_short_cap': None, 'early_short_skew_mult': 1.0, 'early_short_flatten': None}
#   INTARIAN_PEPPER_ROOT: {'position_limit': 50, 'fair_value_method': 'linear_drift', 'fair_value_fallbacks': ('depth_mid', 'hybrid_wall_micro', 'mid'), 'anchor_price': None, 'taker_edge': 2.0, 'maker_edge': 1.0, 'quote_size': 4, 'max_aggressive_size': 8, 'inventory_skew': 2.0, 'flatten_threshold': 0.7, 'history_length': 32, 'taker_edge_buy': 1.5, 'taker_edge_sell': 3.0, 'early_window': 0, 'early_taker_edge_buy': None, 'early_taker_edge_sell': None, 'early_short_cap': None, 'early_short_skew_mult': 1.0, 'early_short_flatten': None}
# ========================================================================
...
def round1_h1_engine_config() -> EngineConfig:
    """Phase-8.5 hybrid H1 candidate — promoted PEPPER + alt ASH.

    Composed of the two already-shipped legs:

    - ASH_COATED_OSMIUM = alt ASH (C-ASH-B): ``wall_mid``,
      ``taker_edge=0.5``, ``maker_edge=1.5``. Empirically the best ASH
      leg on the one official Round-1 data point (+982.81 vs
      promoted's +720.91).
    - INTARIAN_PEPPER_ROOT = promoted PEPPER (C-PEP-A): ``linear_drift``
      h=32, ``taker_edge=2.0``, ``inventory_skew=2.0``,
      ``flatten_threshold=0.7``. The validated low-variance PEPPER leg
      — official near-limit exposure 0 vs alt PEPPER's 22 of 250
      snapshots in bucket 50-75k.

    H1 is an **additive** export — it does not replace promoted or
    alt. See ``outputs/round_1/phase8_5/hybrid_memo.md``.
    """
    return _round1_engine_with(
        ASH_COATED_OSMIUM=dict(
            fair_value_method="wall_mid",
...
class Trader:
    def __init__(
        ...
        *,
        ...
        self.config = config or round1_f5_engine_config()
