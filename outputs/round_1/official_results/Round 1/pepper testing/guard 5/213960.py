...
class ProductConfig:
    ...
    def __post_init__(self) -> None:
        if self.strategy_name not in KNOWN_STRATEGY_NAMES:
            raise ValueError(f'ProductConfig.strategy_name must be one of {_format_known_names(KNOWN_STRATEGY_NAMES)} (got {self.strategy_name!r})')
        if self.fair_value_method not in KNOWN_ESTIMATOR_NAMES:
            raise ValueError(f'ProductConfig.fair_value_method must be one of {_format_known_names(KNOWN_ESTIMATOR_NAMES)} (got {self.fair_value_method!r})')
        unknown_fallbacks = tuple((name for name in self.fair_value_fallbacks if name not in KNOWN_ESTIMATOR_NAMES))
        if unknown_fallbacks:
            raise ValueError(f'ProductConfig.fair_value_fallbacks contains unknown estimator(s) {unknown_fallbacks}; known estimators: {_format_known_names(KNOWN_ESTIMATOR_NAMES)}')
        if self.position_limit <= 0:
            raise ValueError(f'ProductConfig.position_limit must be > 0 (got {self.position_limit})')
        if self.tick_size <= 0:
        ...
        if self.max_aggressive_size < 0:
            raise ValueError(f'ProductConfig.max_aggressive_size must be >= 0 (got {self.max_aggressive_size})')
        ...
        if not 0.0 <= self.flatten_threshold <= 1.0:
            raise ValueError(f'ProductConfig.flatten_threshold must be in [0, 1] (got {self.flatten_threshold})')
        ...
        if 'anchor' in self.fair_value_fallbacks and self.anchor_price is None:
            raise ValueError("ProductConfig: fair_value_fallbacks includes 'anchor' but anchor_price is not set")
        ...
        if self.early_short_flatten is not None and (not 0.0 <= self.early_short_flatten <= 1.0):
            raise ValueError(f'ProductConfig.early_short_flatten must be in [0, 1] when set (got {self.early_short_flatten})')
        if self.early_short_cap is not None and self.early_short_cap > 0:
            raise ValueError(f'ProductConfig.early_short_cap must be <= 0 when set (got {self.early_short_cap}); use 0 to forbid ever going short')
...
class Trader:
    ...
    def __init__(self, config: EngineConfig | None=None, *, state_store: StateStore | None=None, reraise_exceptions: bool=False) -> None:
        self.config = config or round1_pepper_guard005_engine_config()
...
class CoreLongParams:
    ...
    def __post_init__(self) -> None:
        ...
        if self.ceiling < self.floor:
            raise ValueError(f'ceiling must be >= floor (got floor={self.floor}, ceiling={self.ceiling})')
        if self.step <= 0:
            raise ValueError(f'step must be > 0 (got {self.step})')
        if self.exec_style not in EXEC_STYLES:
            raise ValueError(f'exec_style must be one of {EXEC_STYLES!r} (got {self.exec_style!r})')
        ...
        if self.open_seed_size < 0:
            raise ValueError(f'open_seed_size must be >= 0 (got {self.open_seed_size})')
        if self.open_seed_size > self.ceiling:
            raise ValueError(f'open_seed_size ({self.open_seed_size}) must be <= ceiling ({self.ceiling})')
        ...
        if self.open_take_mode not in OPEN_TAKE_MODES:
            raise ValueError(f'open_take_mode must be one of {OPEN_TAKE_MODES!r} (got {self.open_take_mode!r})')
        if self.guard_window < 0:
            raise ValueError(f'guard_window must be >= 0 (got {self.guard_window})')
        if self.guard_negative_slope < 0:
            raise ValueError(f'guard_negative_slope must be >= 0 (got {self.guard_negative_slope})')
        if not 0.0 <= self.guard_r2_min <= 1.0:
            raise ValueError(f'guard_r2_min must be in [0, 1] (got {self.guard_r2_min})')
        guard_enabled = self.guard_window > 0 or self.guard_negative_slope > 0 or self.guard_r2_min > 0
        if guard_enabled and (not self.floor <= self.guard_target <= self.ceiling):
            raise ValueError(f'guard_target must lie inside [floor, ceiling] (got floor={self.floor}, guard_target={self.guard_target}, ceiling={self.ceiling})')
        if self.micro_residual_threshold < 0:
            raise ValueError(f'micro_residual_threshold must be >= 0 (got {self.micro_residual_threshold})')
        if not 0.0 <= self.micro_imbalance_threshold <= 1.0:
            raise ValueError(f'micro_imbalance_threshold must be in [0, 1] (got {self.micro_imbalance_threshold})')
        if self.micro_add_size < 0:
            raise ValueError(f'micro_add_size must be >= 0 (got {self.micro_add_size})')
        if self.micro_trim_size < 0:
            raise ValueError(f'micro_trim_size must be >= 0 (got {self.micro_trim_size})')
        if not 0.0 <= self.adaptive_r2_min <= 1.0:
            raise ValueError(f'adaptive_r2_min must be in [0, 1] (got {self.adaptive_r2_min})')
        if self.adaptive_mid_slope < 0:
            raise ValueError(f'adaptive_mid_slope must be >= 0 (got {self.adaptive_mid_slope})')
        if self.adaptive_high_slope < 0:
            raise ValueError(f'adaptive_high_slope must be >= 0 (got {self.adaptive_high_slope})')
        if self.adaptive_high_slope < self.adaptive_mid_slope:
            raise ValueError(f'adaptive_high_slope must be >= adaptive_mid_slope (got mid={self.adaptive_mid_slope}, high={self.adaptive_high_slope})')
        adaptive_caps = (self.adaptive_low_cap, self.adaptive_mid_cap, self.adaptive_high_cap)
        if any((cap < 0 for cap in adaptive_caps)):
            raise ValueError(f'adaptive_*_cap must be >= 0 (got low={self.adaptive_low_cap}, mid={self.adaptive_mid_cap}, high={self.adaptive_high_cap})')
        if self.adaptive_caps_enabled:
            if self.guard_window <= 0:
                raise ValueError(f'adaptive_caps_enabled requires guard_window > 0 (got {self.guard_window})')
            if not self.floor <= self.adaptive_low_cap <= self.adaptive_mid_cap <= self.adaptive_high_cap:
                raise ValueError(f'adaptive caps must be ordered inside [floor, ceiling] (got floor={self.floor}, low={self.adaptive_low_cap}, mid={self.adaptive_mid_cap}, high={self.adaptive_high_cap}, ceiling={self.ceiling})')
            if self.adaptive_high_cap > self.ceiling:
                raise ValueError(f'adaptive_high_cap must be <= ceiling (got high={self.adaptive_high_cap}, ceiling={self.ceiling})')
...
def round1_pepper_guard005_engine_config() -> EngineConfig:
    return _round1_engine_with(ASH_COATED_OSMIUM=dict(fair_value_method='wall_mid', fair_value_fallbacks=('mid', 'microprice'), maker_edge=1.5, taker_edge=0.5, inventory_skew=4.0, flatten_threshold=0.7, history_length=48), INTARIAN_PEPPER_ROOT=dict(strategy_name='pepper_core_long', fair_value_method='linear_drift', fair_value_fallbacks=('depth_mid', 'hybrid_wall_micro', 'mid'), maker_edge=1.0, taker_edge=2.0, quote_size=10, max_aggressive_size=20, inventory_skew=2.0, flatten_threshold=0.7, history_length=32))
_PEPPER_GUARD005_CORE_LONG_PARAMS = CoreLongParams(base_long=80, add_thresh=3.0, trim_thresh=8.0, add_gain=5.0, trim_gain=2.0, floor=0, ceiling=80, step=8, exec_style='taker', hybrid_threshold=2.0, maker_edge_offset=0.0, open_seed_size=65, open_window=500, open_no_short=True, open_take_mode='level1_only', guard_window=32, guard_negative_slope=0.005, guard_r2_min=0.0, guard_target=0, micro_residual_threshold=0.0, micro_imbalance_threshold=1.0, micro_add_size=0, micro_trim_size=0, adaptive_caps_enabled=False, adaptive_r2_min=0.0, adaptive_mid_slope=0.0, adaptive_high_slope=0.0, adaptive_low_cap=0, adaptive_mid_cap=0, adaptive_high_cap=0)

def _pepper_guard005_pepper_strategy_factory(fve: FairValueEngine, sig: SignalEngine) -> BaseStrategy:
    return PepperCoreLongStrategy(fve, sig, _PEPPER_GUARD005_CORE_LONG_PARAMS)
KNOWN_STRATEGY_NAMES = tuple(sorted(set(KNOWN_STRATEGY_NAMES) | {'pepper_core_long'}))
STRATEGY_REGISTRY = MappingProxyType(dict(STRATEGY_REGISTRY, pepper_core_long=_pepper_guard005_pepper_strategy_factory))