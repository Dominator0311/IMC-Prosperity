...
# ========================================================================
# ROUND-1 UPLOAD VARIANT: pepper_drift_asymmetric
# Label   : Round-1 PEPPER Fodra-Labadie asymmetric-maker candidate
# Purpose : Asymmetric maker around linear_drift forecast; edge asymmetry scales with observed slope (slope_skew_factor=10, max_asymmetry=3). Reversal guard flips target to 0 when slope <= -0.02 with r2 >= 0.30. Local 3-day PEPPER mean +39,237.
# Factory : src.core.config.round1_pepper_drift_asymmetric_engine_config
# Built   : 2026-04-16T19:43:30.498233+00:00 (commit dfcb239)
...
# PEPPER strategy: PepperDriftAsymmetricStrategy (research-only, inlined)
# PEPPER params: {'base_edge': 3.0, 'quote_size': 5, 'slope_skew_factor': 10.0, 'max_asymmetry': 3.0, 'slope_window': 32, 'slope_r2_min': 0.3, 'inventory_skew_coef': 0.05, 'core_target': 50, 'floor': 0, 'ceiling': 80, 'seed_size': 40, 'seed_window': 500, 'reversal_slope_threshold': 0.02, 'reversal_r2_min': 0.3, 'reversal_target': 0, 'min_spread_for_maker': 4}
# ========================================================================
...
class Trader:
    def __init__(
        ...
        *,
        ...
        self.config = config or round1_pepper_drift_asymmetric_engine_config()
...
_ASH_PEPPER_DRIFT_ASYMMETRIC_PARAMS = LadderParams(
    edges=(2.5, 4.0, 6.0),
...
def _ash_pepper_drift_asymmetric_factory(fve: FairValueEngine, sig: SignalEngine) -> BaseStrategy:
    return AshLadderStrategy(fve, sig, _ASH_PEPPER_DRIFT_ASYMMETRIC_PARAMS)

_PEP_PEPPER_DRIFT_ASYMMETRIC_PARAMS = DriftAsymmetricParams(
    base_edge=3.0,
    quote_size=5,
    ...
    seed_size=40,
    seed_window=500,
    reversal_slope_threshold=0.02,
    reversal_r2_min=0.3,
    reversal_target=0,
    min_spread_for_maker=4,
)

def _pep_pepper_drift_asymmetric_factory(fve: FairValueEngine, sig: SignalEngine) -> BaseStrategy:
    return PepperDriftAsymmetricStrategy(fve, sig, _PEP_PEPPER_DRIFT_ASYMMETRIC_PARAMS)

KNOWN_STRATEGY_NAMES = tuple(
    sorted(set(KNOWN_STRATEGY_NAMES) | {"ash_ladder", "pepper_drift_asymmetric"})
)
STRATEGY_REGISTRY = MappingProxyType(
    dict(
        STRATEGY_REGISTRY,
        **{
            "ash_ladder": _ash_pepper_drift_asymmetric_factory,
            "pepper_drift_asymmetric": _pep_pepper_drift_asymmetric_factory,
        }
    )
)
