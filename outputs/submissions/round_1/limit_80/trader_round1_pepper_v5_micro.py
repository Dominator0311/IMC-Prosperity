...
class Trader:
    ...
    def __init__(self, config: EngineConfig | None=None, *, state_store: StateStore | None=None, reraise_exceptions: bool=False) -> None:
        self.config = config or round1_pepper_v5_micro_engine_config()
...
def round1_pepper_v5_micro_engine_config() -> EngineConfig:
...
_PEPPER_V5_MICRO_CORE_LONG_PARAMS = CoreLongParams(base_long=80, add_thresh=3.0, trim_thresh=8.0, add_gain=5.0, trim_gain=2.0, floor=0, ceiling=80, step=8, exec_style='taker', hybrid_threshold=2.0, maker_edge_offset=0.0, open_seed_size=65, open_window=500, open_no_short=True, open_take_mode='level1_only', guard_window=32, guard_negative_slope=0.01, guard_r2_min=0.0, guard_target=0, micro_residual_threshold=3.0, micro_imbalance_threshold=0.3, micro_add_size=2, micro_trim_size=2, adaptive_caps_enabled=False, adaptive_r2_min=0.0, adaptive_mid_slope=0.0, adaptive_high_slope=0.0, adaptive_low_cap=0, adaptive_mid_cap=0, adaptive_high_cap=0)

def _pepper_v5_micro_pepper_strategy_factory(fve: FairValueEngine, sig: SignalEngine) -> BaseStrategy:
    return PepperCoreLongStrategy(fve, sig, _PEPPER_V5_MICRO_CORE_LONG_PARAMS)
KNOWN_STRATEGY_NAMES = tuple(sorted(set(KNOWN_STRATEGY_NAMES) | {'pepper_core_long'}))
STRATEGY_REGISTRY = MappingProxyType(dict(STRATEGY_REGISTRY, pepper_core_long=_pepper_v5_micro_pepper_strategy_factory))
