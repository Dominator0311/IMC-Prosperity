...
# ========================================================================
# ROUND-1 UPLOAD VARIANT: ash_deep_l6
# Label   : Round-1 Phase-L L6 — K2 with weights 5/2/2 (55% inner)
# Purpose : K2 edges (2.5/4/6) with weights (5, 2, 2) = inner 55.6%, outers 22.2% each. Tests whether shifting slightly more weight toward outers (vs K2's 60%/20%/20%) captures more outer-level edge without losing inner coverage. Local +3 515 (vs K2 +3 497). Close call — officially could go either way.
# Factory : src.core.config.round1_ash_deep_l6_engine_config
# Built   : 2026-04-16T18:46:03.795750+00:00 (commit bb8570c)
...
# ASH params: {'edges': (2.5, 4.0, 6.0), 'size_mults': (1.0, 1.5, 2.0), 'skew_coef': 2.0, 'flatten_threshold': 0.7, 'weights': (5, 2, 2)}
# ========================================================================
...
class Trader:
    def __init__(
        ...
        *,
        ...
        self.config = config or round1_ash_deep_l6_engine_config()
...
_ASH_DEEP_L6_PARAMS = LadderParams(
    edges=(2.5, 4.0, 6.0),
...
def _ash_deep_l6_ash_strategy_factory(
    fve: FairValueEngine, sig: SignalEngine
) -> BaseStrategy:
    return AshLadderStrategy(fve, sig, _ASH_DEEP_L6_PARAMS)

KNOWN_STRATEGY_NAMES = tuple(
    sorted(set(KNOWN_STRATEGY_NAMES) | {"ash_ladder"})
)
STRATEGY_REGISTRY = MappingProxyType(
    dict(STRATEGY_REGISTRY, ash_ladder=_ash_deep_l6_ash_strategy_factory)
)
