...
# ========================================================================
# ROUND-1 UPLOAD VARIANT: pepper_flow_overlay
# Label   : Round-1 PEPPER trade-flow EWMA overlay candidate
# Purpose : Reads snapshot.trades (aggressor flow) — untouched by any prior PEPPER strategy. EWMA of net aggressor flow biases target position up to +/-20 units around core_long=50. Local 3-day PEPPER mean +51,752.
# Factory : src.core.config.round1_pepper_flow_overlay_engine_config
# Built   : 2026-04-16T19:43:30.924234+00:00 (commit dfcb239)
...
# PEPPER strategy: PepperFlowOverlayStrategy (research-only, inlined)
# PEPPER params: {'core_long': 50, 'floor': 0, 'ceiling': 80, 'step': 8, 'flow_decay': 0.85, 'flow_scale': 0.5, 'flow_bias_size': 20, 'flow_min_magnitude': 2.0, 'taker_edge': 1.5, 'maker_bid_edge': 3.0, 'maker_ask_edge': 5.0, 'maker_quote_size': 3, 'seed_size': 40, 'seed_window': 500, 'min_spread_for_maker': 4, 'flow_ewma_key': 'pepper_flow_ewma', 'prior_mid_key': 'pepper_flow_prior_mid'}
# ========================================================================
...
class Trader:
    def __init__(
        ...
        *,
        ...
        self.config = config or round1_pepper_flow_overlay_engine_config()
...
_ASH_PEPPER_FLOW_OVERLAY_PARAMS = LadderParams(
    edges=(2.5, 4.0, 6.0),
...
def _ash_pepper_flow_overlay_factory(fve: FairValueEngine, sig: SignalEngine) -> BaseStrategy:
    return AshLadderStrategy(fve, sig, _ASH_PEPPER_FLOW_OVERLAY_PARAMS)

_PEP_PEPPER_FLOW_OVERLAY_PARAMS = FlowOverlayParams(
    core_long=50,
    floor=0,
    ceiling=80,
    step=8,
    ...
    flow_ewma_key='pepper_flow_ewma',
    prior_mid_key='pepper_flow_prior_mid',
)

def _pep_pepper_flow_overlay_factory(fve: FairValueEngine, sig: SignalEngine) -> BaseStrategy:
    return PepperFlowOverlayStrategy(fve, sig, _PEP_PEPPER_FLOW_OVERLAY_PARAMS)

KNOWN_STRATEGY_NAMES = tuple(
    sorted(set(KNOWN_STRATEGY_NAMES) | {"ash_ladder", "pepper_flow_overlay"})
)
STRATEGY_REGISTRY = MappingProxyType(
    dict(
        STRATEGY_REGISTRY,
        **{
            "ash_ladder": _ash_pepper_flow_overlay_factory,
            "pepper_flow_overlay": _pep_pepper_flow_overlay_factory,
        }
    )
)
