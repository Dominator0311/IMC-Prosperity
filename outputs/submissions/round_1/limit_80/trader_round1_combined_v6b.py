...
_ASH_COMBINED_V6B_PARAMS = LadderParams(edges=(2.5, 3.5, 5.0), size_mults=(1.0, 1.5, 2.0), skew_coef=2.0, flatten_threshold=0.7, weights=(3, 1, 1))

def _ash_combined_v6b_factory(fve: FairValueEngine, sig: SignalEngine) -> BaseStrategy:
    return AshLadderStrategy(fve, sig, _ASH_COMBINED_V6B_PARAMS)
_PEP_COMBINED_V6B_PARAMS = V6CombinedParams(base_long=65, floor=0, ceiling=80, step=8, opening_passive_window=3, opening_seed_size=50, opening_taker_fallback_tick=3, opening_no_short=True, opening_max_size_per_tick=20, passive_bid_improve=1, add_thresh=3.0, trim_thresh=8.0, add_gain=5.0, trim_gain=2.0, guard_window=32, guard_negative_slope=0.01, guard_r2_min=0.0, guard_target=0, micro_residual_threshold=3.0, micro_imbalance_threshold=0.3, micro_add_size=2, micro_trim_size=2, maker_base_bid_edge=3.0, maker_base_ask_edge=5.0, maker_quote_size=5, maker_inventory_skew_coef=0.04, min_spread_for_maker=4, drift_slope_skew_factor=10.0, drift_max_asymmetry=3.0, drift_slope_r2_min=0.3, drift_reversal_slope_threshold=0.02, drift_reversal_r2_min=0.3, drift_reversal_target=0, exec_style='hybrid', hybrid_threshold=2.0)

def _pep_combined_v6b_factory(fve: FairValueEngine, sig: SignalEngine) -> BaseStrategy:
    return PepperV6CombinedStrategy(fve, sig, _PEP_COMBINED_V6B_PARAMS)
KNOWN_STRATEGY_NAMES = tuple(sorted(set(KNOWN_STRATEGY_NAMES) | {'ash_ladder', 'pepper_v6_combined'}))
STRATEGY_REGISTRY = MappingProxyType(dict(STRATEGY_REGISTRY, **{'ash_ladder': _ash_combined_v6b_factory, 'pepper_v6_combined': _pep_combined_v6b_factory}))
