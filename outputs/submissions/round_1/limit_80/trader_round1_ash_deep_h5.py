...
# ========================================================================
# ROUND-1 UPLOAD VARIANT: ash_deep_h5
# Label   : Round-1 Phase-H H5 — slow-FV anchor (ewma τ=500)
# Purpose : Phase-H hidden-pattern test. ASH Roll (1984) decomposition shows fundamental vol ~0 and OU half-life 2 ticks around a near-constant FV. F3a uses weighted_mid which tracks the OU state (noise); H5 replaces FV with ewma τ=500 which tracks the OU mean. Local 3-day mean +1 908 (below F3a +3 446) but fill-scale projected official +1 639 (+244 vs F3a observed +1 395) because maker fills are ~5x denser. Resolves local-vs-projection ambiguity: if H5 wins the slow-FV hypothesis is confirmed; if it loses F3a is proven optimal.
# Factory : src.core.config.round1_ash_deep_h5_engine_config
# Built   : 2026-04-16T16:24:53.087644+00:00 (commit 4dbca96)
# Embedded product configs:
#   ASH_COATED_OSMIUM: {'position_limit': 80, 'strategy_name': 'ash_shape_override', 'fair_value_method': 'ewma_mid', 'fair_value_fallbacks': ('wall_mid', 'mid'), 'anchor_price': 10000.0, 'taker_edge': 0.5, 'maker_edge': 2.5, 'quote_size': 5, 'max_aggressive_size': 10, 'inventory_skew': 4.0, 'flatten_threshold': 0.7, 'history_length': 2000, 'taker_edge_buy': None, 'taker_edge_sell': None, 'early_window': 0, 'early_taker_edge_buy': None, 'early_taker_edge_sell': None, 'early_short_cap': None, 'early_short_skew_mult': 1.0, 'early_short_flatten': None}
...
def round1_ash_deep_h5_engine_config() -> EngineConfig:
    import math
    ...
    return _round1_engine_with(
        ASH_COATED_OSMIUM=dict(
            strategy_name="ash_shape_override",
            fair_value_method="ewma_mid",
...
class Trader:
    def __init__(
        ...
        *,
        ...
        self.config = config or round1_ash_deep_h5_engine_config()
...
_ASH_DEEP_H5_PARAMS = ShapeParams(
    skew_mode='linear',
    skew_coef=2.0,
    flatten_mode='hard',
    flatten_threshold=0.7,
...
def _ash_deep_h5_ash_strategy_factory(
    fve: FairValueEngine, sig: SignalEngine
) -> BaseStrategy:
    return AshShapeOverrideStrategy(fve, sig, _ASH_DEEP_H5_PARAMS)

KNOWN_STRATEGY_NAMES = tuple(
    sorted(set(KNOWN_STRATEGY_NAMES) | {"ash_shape_override"})
)
STRATEGY_REGISTRY = MappingProxyType(
    dict(STRATEGY_REGISTRY, ash_shape_override=_ash_deep_h5_ash_strategy_factory)
)
