...
# ========================================================================
# ROUND-1 UPLOAD VARIANT: ash_deep_k2
# Label   : Round-1 Phase-K K2 — J2_tight (outer at 4/6)
# Purpose : J2 variant with tighter outer edges (4, 6 instead of 5, 8). Rationale: ASH spread is dominantly 16 (each side ~8 ticks); outer quotes at edge=6 still sit inside the book typically, while edge=8 might miss. Local +3 497 (vs J2 +3 653). If edge=4,6 fires at higher official rate than edge=5,8, could improve PnL despite lower per-share edge.
# Factory : src.core.config.round1_ash_deep_k2_engine_config
# Built   : 2026-04-16T18:16:45.148315+00:00 (commit 16f3db7)
...
class Trader:
    def __init__(
        ...
        *,
        ...
        self.config = config or round1_ash_deep_k2_engine_config()
...
_ASH_DEEP_K2_PARAMS = LadderParams(
    edges=(2.5, 4.0, 6.0),
...
def _ash_deep_k2_ash_strategy_factory(
    fve: FairValueEngine, sig: SignalEngine
) -> BaseStrategy:
    return AshLadderStrategy(fve, sig, _ASH_DEEP_K2_PARAMS)

KNOWN_STRATEGY_NAMES = tuple(
    sorted(set(KNOWN_STRATEGY_NAMES) | {"ash_ladder"})
)
STRATEGY_REGISTRY = MappingProxyType(
    dict(STRATEGY_REGISTRY, ash_ladder=_ash_deep_k2_ash_strategy_factory)
)
