...
# ========================================================================
# ROUND-1 UPLOAD VARIANT: ash_deep_k5
# Label   : Round-1 Phase-K K5 — asymmetric ladder (buy tight, sell wide)
# Purpose : First asymmetric per-side ladder. Buy side uses tighter edges (2.5/4/6) to accumulate quickly on dips; sell side uses wider edges (2.5/5/8) to hold for bigger reversions. Rationale: if the OU process has asymmetric mean-reversion (e.g., sharp dips with slower recoveries), per-side depths better match the process. Local +3 581 (vs J2 +3 653).
# Factory : src.core.config.round1_ash_deep_k5_engine_config
# Built   : 2026-04-16T18:16:45.575870+00:00 (commit 16f3db7)
...
# ASH params: {'edges': (2.5, 5.0, 8.0), 'size_mults': (1.0, 1.5, 2.0), 'skew_coef': 2.0, 'flatten_threshold': 0.7, 'weights': (3, 1, 1), 'buy_edges': (2.5, 4.0, 6.0), 'buy_size_mults': (1.0, 1.5, 2.0), 'buy_weights': (3, 1, 1), 'sell_edges': (2.5, 5.0, 8.0), 'sell_size_mults': (1.0, 1.5, 2.0), 'sell_weights': (3, 1, 1)}
# ========================================================================
...
class Trader:
    def __init__(
        ...
        *,
        ...
        self.config = config or round1_ash_deep_k5_engine_config()
...
_ASH_DEEP_K5_PARAMS = LadderParams(
    edges=(2.5, 5.0, 8.0),
...
def _ash_deep_k5_ash_strategy_factory(
    fve: FairValueEngine, sig: SignalEngine
) -> BaseStrategy:
    return AshLadderStrategy(fve, sig, _ASH_DEEP_K5_PARAMS)

KNOWN_STRATEGY_NAMES = tuple(
    sorted(set(KNOWN_STRATEGY_NAMES) | {"ash_ladder"})
)
STRATEGY_REGISTRY = MappingProxyType(
    dict(STRATEGY_REGISTRY, ash_ladder=_ash_deep_k5_ash_strategy_factory)
)
