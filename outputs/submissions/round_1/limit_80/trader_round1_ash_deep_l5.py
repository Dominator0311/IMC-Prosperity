...
# ========================================================================
# ROUND-1 UPLOAD VARIANT: ash_deep_l5
# Label   : Round-1 Phase-L L5 — K2 with big outer sizes (1/3/5)
# Purpose : K2 edges (2.5/4/6) with size_mults (1, 3, 5) — outer levels quote 3x and 5x the inner size. Hypothesis: outer fills are rare but high-edge; making each fill bigger captures more PnL per event. Local identical to K2 (+3 497) because sim caps maker fill size — should diverge officially where counterparty depth allows larger fills.
# Factory : src.core.config.round1_ash_deep_l5_engine_config
# Built   : 2026-04-16T18:46:01.645305+00:00 (commit bb8570c)
...
# ASH params: {'edges': (2.5, 4.0, 6.0), 'size_mults': (1.0, 3.0, 5.0), 'skew_coef': 2.0, 'flatten_threshold': 0.7, 'weights': (3, 1, 1)}
# ========================================================================
...
class Trader:
    def __init__(
        ...
        *,
        ...
        self.config = config or round1_ash_deep_l5_engine_config()
...
_ASH_DEEP_L5_PARAMS = LadderParams(
    edges=(2.5, 4.0, 6.0),
...
def _ash_deep_l5_ash_strategy_factory(
    fve: FairValueEngine, sig: SignalEngine
) -> BaseStrategy:
    return AshLadderStrategy(fve, sig, _ASH_DEEP_L5_PARAMS)

KNOWN_STRATEGY_NAMES = tuple(
    sorted(set(KNOWN_STRATEGY_NAMES) | {"ash_ladder"})
)
STRATEGY_REGISTRY = MappingProxyType(
    dict(STRATEGY_REGISTRY, ash_ladder=_ash_deep_l5_ash_strategy_factory)
)
