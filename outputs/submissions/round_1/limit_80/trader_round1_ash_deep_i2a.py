...
# ========================================================================
# ROUND-1 UPLOAD VARIANT: ash_deep_i2a
# Label   : Round-1 Phase-I I2a — 4-level ladder (2.5/5/8/12)
# Purpose : Phase-I post-Phase-H exploration. F3a's engine with a tick-rotation maker ladder: each tick quotes at one of four edge levels (2.5, 5, 8, 12) with sizes scaled (1x, 1.5x, 2x, 3x). Local 3-day mean +3 666 (vs F3a +3 446, Δ +220 — first variant beating F3a on local). 14x maker fill count (72 vs 5) at higher per-fill markout (+2.11). Hypothesis: multi-level quoting captures the OU amplitude distribution F3a's single-edge misses.
# Factory : src.core.config.round1_ash_deep_i2a_engine_config
# Built   : 2026-04-16T17:18:28.537836+00:00 (commit 043faac)
...
# ASH params: {'edges': (2.5, 5.0, 8.0, 12.0), 'size_mults': (1.0, 1.5, 2.0, 3.0), 'skew_coef': 2.0, 'flatten_threshold': 0.7}
# ========================================================================
...
class Trader:
    def __init__(
        ...
        *,
        ...
        self.config = config or round1_ash_deep_i2a_engine_config()
...
class AshLadderStrategy(BaseStrategy):
    ...
    def generate_intent(self, context: StrategyContext) -> SignalIntent:
        ...
        level = counter % len(params.edges)
        ...
        raw_bid = math.floor(fair_value.price - edge - skew)
        raw_ask = math.ceil(fair_value.price + edge - skew)
...
_ASH_DEEP_I2A_PARAMS = LadderParams(
...
def _ash_deep_i2a_ash_strategy_factory(
    fve: FairValueEngine, sig: SignalEngine
) -> BaseStrategy:
    return AshLadderStrategy(fve, sig, _ASH_DEEP_I2A_PARAMS)

KNOWN_STRATEGY_NAMES = tuple(
    sorted(set(KNOWN_STRATEGY_NAMES) | {"ash_ladder"})
)
STRATEGY_REGISTRY = MappingProxyType(
    dict(STRATEGY_REGISTRY, ash_ladder=_ash_deep_i2a_ash_strategy_factory)
)
