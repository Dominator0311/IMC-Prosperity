...
# ========================================================================
# ROUND-1 UPLOAD VARIANT: ash_deep_g5
# Label   : Round-1 Phase-G G5 — F3a with skew c=1 (softer)
# Purpose : Phase-G extension. F3a's engine config (weighted_mid + m=2.5/t=0.5) with linear skew coefficient halved to 1.0 (F3a uses c=2). Phase-G local sweep showed c=1 as the local optimum along the skew axis (+3 495 3-day mean, highest of any ASH variant tested). Projected official +1 468 (Δ +72 vs F3a).
# Factory : src.core.config.round1_ash_deep_g5_engine_config
# Built   : 2026-04-16T15:30:06.494216+00:00 (commit 043faac)
# Embedded product configs:
#   ASH_COATED_OSMIUM: {'position_limit': 80, 'strategy_name': 'ash_shape_override', 'fair_value_method': 'weighted_mid', 'fair_value_fallbacks': ('wall_mid', 'mid'), 'anchor_price': 10000.0, 'taker_edge': 0.5, 'maker_edge': 2.5, 'quote_size': 5, 'max_aggressive_size': 10, 'inventory_skew': 4.0, 'flatten_threshold': 0.7, 'history_length': 48, 'taker_edge_buy': None, 'taker_edge_sell': None, 'early_window': 0, 'early_taker_edge_buy': None, 'early_taker_edge_sell': None, 'early_short_cap': None, 'early_short_skew_mult': 1.0, 'early_short_flatten': None}
...
# ASH strategy: AshShapeOverrideStrategy (research-only, inlined at bundle tail; NOT in shipped STRATEGY_REGISTRY)
# ASH params: {'skew_mode': 'linear', 'skew_coef': 1.0, 'flatten_mode': 'hard', 'flatten_threshold': 0.7, 'size_mode': 'constant'}
# ========================================================================
...
round1_ash_deep_g1_engine_config = round1_ash_deep_f2b_engine_config
...
round1_ash_deep_g5_engine_config = round1_ash_deep_f3a_engine_config
...
class Trader:
    def __init__(
        ...
        *,
        ...
        self.config = config or round1_ash_deep_g5_engine_config()
...
_ASH_DEEP_G5_PARAMS = ShapeParams(
    skew_mode='linear',
    skew_coef=1.0,
    flatten_mode='hard',
    flatten_threshold=0.7,
...
def _ash_deep_g5_ash_strategy_factory(
    fve: FairValueEngine, sig: SignalEngine
) -> BaseStrategy:
    return AshShapeOverrideStrategy(fve, sig, _ASH_DEEP_G5_PARAMS)

KNOWN_STRATEGY_NAMES = tuple(
    sorted(set(KNOWN_STRATEGY_NAMES) | {"ash_shape_override"})
)
STRATEGY_REGISTRY = MappingProxyType(
    dict(STRATEGY_REGISTRY, ash_shape_override=_ash_deep_g5_ash_strategy_factory)
)
