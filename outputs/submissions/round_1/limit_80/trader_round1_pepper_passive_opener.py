...
# ========================================================================
# ROUND-1 UPLOAD VARIANT: pepper_passive_opener
# Label   : Round-1 PEPPER passive-first opening candidate
# Purpose : Replaces V3's taker-first opening with a 3-tick passive-first open (bid at best_bid+1) + taker fallback. Designed to save ~200-400 PnL of entry spread cost on the 40-unit seed. Post-open: drift-maker carry with asymmetric inventory-skewed edges. Local 3-day PEPPER mean +66,373.
# Factory : src.core.config.round1_pepper_passive_opener_engine_config
# Built   : 2026-04-16T19:43:31.106332+00:00 (commit dfcb239)
...
# PEPPER strategy: PepperPassiveOpenerStrategy (research-only, inlined)
# PEPPER params: {'opening_passive_window': 3, 'opening_taker_fallback_tick': 3, 'passive_bid_improve': 1, 'opening_max_size_per_tick': 20, 'seed_size': 40, 'steady_core_target': 40, 'maker_bid_edge': 3.0, 'maker_ask_edge': 5.0, 'maker_quote_size': 5, 'inventory_skew_coef': 0.04, 'floor': 0, 'ceiling': 80, 'min_spread_for_maker': 4, 'opener_tick_key': 'passive_opener_ticks'}
# ========================================================================
...
class Trader:
    def __init__(
        ...
        *,
        ...
        self.config = config or round1_pepper_passive_opener_engine_config()
...
_ASH_PEPPER_PASSIVE_OPENER_PARAMS = LadderParams(
    edges=(2.5, 4.0, 6.0),
...
def _ash_pepper_passive_opener_factory(fve: FairValueEngine, sig: SignalEngine) -> BaseStrategy:
    return AshLadderStrategy(fve, sig, _ASH_PEPPER_PASSIVE_OPENER_PARAMS)

_PEP_PEPPER_PASSIVE_OPENER_PARAMS = PassiveOpenerParams(
    ...
    opener_tick_key='passive_opener_ticks',
)

def _pep_pepper_passive_opener_factory(fve: FairValueEngine, sig: SignalEngine) -> BaseStrategy:
    return PepperPassiveOpenerStrategy(fve, sig, _PEP_PEPPER_PASSIVE_OPENER_PARAMS)

KNOWN_STRATEGY_NAMES = tuple(
    sorted(set(KNOWN_STRATEGY_NAMES) | {"ash_ladder", "pepper_passive_opener"})
)
STRATEGY_REGISTRY = MappingProxyType(
    dict(
        STRATEGY_REGISTRY,
        **{
            "ash_ladder": _ash_pepper_passive_opener_factory,
            "pepper_passive_opener": _pep_pepper_passive_opener_factory,
        }
    )
)
