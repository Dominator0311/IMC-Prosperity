...
class PepperCoreLongStrategy(BaseStrategy):
    ...
    def generate_intent(self, context: StrategyContext) -> SignalIntent:
        ...
        return SignalIntent(product=product, fair_value=fair_value, mode=mode, buy_below=buy_below, sell_above=sell_above, quote=quote, rationale=rationale, metadata=MappingProxyType(metadata))
_ASH_ROUND2_KILLSWITCHES_ON_PARAMS = LadderParams(edges=(3.0, 5.0, 8.0), size_mults=(1.0, 2.0, 3.0), weights=(1, 1, 3), skew_coef=1.0, flatten_threshold=0.7)

def _ash_round2_killswitches_on_factory(fve: FairValueEngine, sig: SignalEngine) -> BaseStrategy:
    return AshLadderStrategy(fve, sig, _ASH_ROUND2_KILLSWITCHES_ON_PARAMS)
_PEP_ROUND2_KILLSWITCHES_ON_PARAMS = CoreLongParams(base_long=80, add_thresh=3.0, trim_thresh=8.0, add_gain=5.0, trim_gain=2.0, floor=0, ceiling=80, step=8, exec_style='taker', hybrid_threshold=2.0, maker_edge_offset=0.0, open_seed_size=65, open_window=500, open_no_short=True, open_take_mode='level1_only', guard_window=32, guard_negative_slope=0.01, guard_r2_min=0.0, guard_target=0, micro_residual_threshold=3.0, micro_imbalance_threshold=0.3, micro_add_size=2, micro_trim_size=2, adaptive_caps_enabled=False, adaptive_r2_min=0.0, adaptive_mid_slope=0.0, adaptive_high_slope=0.0, adaptive_low_cap=0, adaptive_mid_cap=0, adaptive_high_cap=0, kill_slope_window=50, kill_consecutive_neg_slope_n=20, kill_slope_pause_snaps=50, kill_residual_threshold=35.0, kill_residual_release=15.0, kill_step_move_threshold=40.0, kill_step_move_pause_snaps=10, kill_intraday_pnl_threshold=2500.0)

def _pep_round2_killswitches_on_factory(fve: FairValueEngine, sig: SignalEngine) -> BaseStrategy:
    return PepperCoreLongStrategy(fve, sig, _PEP_ROUND2_KILLSWITCHES_ON_PARAMS)
KNOWN_STRATEGY_NAMES = tuple(sorted(set(KNOWN_STRATEGY_NAMES) | {'ash_ladder', 'pepper_core_long'}))
STRATEGY_REGISTRY = MappingProxyType(dict(STRATEGY_REGISTRY, **{'ash_ladder': _ash_round2_killswitches_on_factory, 'pepper_core_long': _pep_round2_killswitches_on_factory}))
