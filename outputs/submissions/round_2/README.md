...
| **`500`** | **Recommended for final upload** (revised after seeing official-test data). |

#### Revised bid: `500` (was `2300`)
...
- **Chosen bid: `500`** → mixture EV +2 125 XIRECs. ~95 EV (~4%)
  worse than the strict EV-max of 350, but rounder, sits comfortably
  above the balanced prior's [50, 500] uniform band so it sweeps
  100% of balanced opponents (vs 97% at 350), and gives a small
  amount of headroom against modest cluster-bidding behaviour.
...
- Bid `2300` (old recommendation): EV ≈ +691 XIRECs at v=3k —
  positive but ~3× worse than bid 500.
...
v=2 000, bid 500 yields +1 286 EV vs +0 for bid 0.

**Why 500 over 1800?** 500 captures the naive-heavy AND balanced
opponent classes (combined ~85% of mixture under default weights,
all bidding < 500) for low cost. The robust bid 1800 only matters
under the aggressive-prior tail (~15% probability) and the
consensus-collapse stress, both speculative — we'd rather expose
+500 of dead-cost in the worst case than pay +1 300 of certain
...
| Bundle SHA256 (`--bid 500`) | `9f4525fe7e3e5151a96b24501ea6f032f32acec7ebbdf4b434b613827585d047` |
| Bundle SHA256 (`--bid 350`, alt EV-max) | `885d9cec6eeda3c06f3a88ffd55ac3f150b8180ffbe4cc8da8f51fbeba3a2218` |
| Bundle SHA256 (`--bid 2300`, deprecated) | `a4a17c8e66e00dde6f67fb7ef970a54f8d3a402d5f90348fee7e18d37540325c` |
| Smoke test | `Trader().bid()` returns 500 ✅; `Trader.run()` produces well-formed orders for both products ✅; pytest `tests/test_round2_export_e2e.py` 19/19 ✅; full pytest suite 733/733 ✅ |
