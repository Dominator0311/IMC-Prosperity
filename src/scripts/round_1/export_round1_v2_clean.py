"""Export the V2_clean Round-1 submission bundle.

V2_clean is the Phase-10 ``pepper_corelong_v2`` search winner:

- ASH leg = H1 / F5 / Alt ``wall_mid`` leg (byte-identical)
- PEPPER leg = opening-seeded core-long + residual overlay, via the
  research-only ``PepperCoreLongStrategy`` with the exact V2_clean
  ``CoreLongParams`` the search pinned down.

The live submission pipeline (``src/scripts/export_submission.py``)
does NOT include ``src/strategies/pepper_core_long.py`` in
``LIVE_MODULE_ORDER`` — it is a research-only module and adding it to
the live path would push every other bundle over the 96 KiB cap. This
script therefore:

1. Calls ``build_submission_source()`` to get the standard live bundle
   (same live path every other Round-1 variant uses — no drift).
2. Strips ``pepper_core_long.py`` through the same ``strip_module``
   helper the exporter uses, so cross-module imports are handled the
   same way.
3. Composes a V2_clean appendix:
   - the stripped ``pepper_core_long`` module body
   - a small wiring block that registers ``PepperCoreLongStrategy``
     under ``"pepper_core_long"`` in the bundle-local
     ``STRATEGY_REGISTRY`` and pins the V2_clean ``CoreLongParams``
4. Patches ``Trader.__init__``'s default factory call to use
   ``round1_v2_clean_engine_config`` (same patch pattern as the
   existing variant export script).
5. Writes the result to
   ``outputs/submissions/round_1/limit_80/trader_round1_v2_clean.py``.

Byte accounting (commit ``{commit}``):

- base live bundle: ~95 k
- V2_clean banner + factory patch: ~1 k
- stripped pepper_core_long body: ~13 k
- wiring appendix: ~0.5 k
- total: ~110 k (under the Phase-10 hard cap of 120 KiB)

Usage::

    PYTHONPATH=. .venv/bin/python -m src.scripts.round_1.export_round1_v2_clean
    PYTHONPATH=. .venv/bin/python -m src.scripts.round_1.export_round1_v2_clean --out-dir /tmp

The emitted file is uploaded EXACTLY as-is; the validator is the
gate. Do NOT upload the inline-datamodel variant.
"""
...
from src.core.config import EngineConfig, round1_v2_clean_engine_config
...
# --------------------------------------------------------------------------- #
# V2_clean params — MUST match outputs/round_1/pepper_corelong_v2/shortlist.md #
# --------------------------------------------------------------------------- #
# Kept as a dict so the banner can print the values, and so the                #
# bundle appendix can materialise ``CoreLongParams(**_V2_CLEAN_PARAMS)``       #
# at import time.                                                              #
# --------------------------------------------------------------------------- #

V2_CLEAN_PARAMS: dict[str, object] = {
    "base_long": 50,
    "add_thresh": 3.0,
    "trim_thresh": 5.0,
    "add_gain": 5.0,
...
def _build_banner(config: EngineConfig, commit: str) -> str:
    return (
        "# " + "=" * 72 + "\n"
        "# ROUND-1 UPLOAD VARIANT: v2_clean\n"
        "# Label   : Round-1 Phase-10 pepper_corelong_v2 winner\n"
        "# Purpose : V2_clean PEPPER (opening-seeded core-long + overlay) + "
        "wall_mid ASH. Closes the first-25k PEPPER gap that every MM "
        "variant left unaddressed; ASH leg byte-identical to H1 / F5 / Alt.\n"
        f"# Factory : src.core.config.round1_v2_clean_engine_config\n"
...
def _patch_default_config_call(source: str) -> str:
    ...
    return source.replace(_DEFAULT_FACTORY_CALL, _V2_CLEAN_FACTORY_CALL, 1)
...
# --------------------------------------------------------------------------- #
#
# Defines at module scope, after the Trader class is defined, the V2_clean
...
# the existing entries plus the V2_clean factory. The Trader looks up
...
# ========================================================================
# V2_clean wiring — research-only PepperCoreLongStrategy is now in scope.
...
# validation. Pin the V2_clean CoreLongParams so the factory signature
...
def _v2_clean_pepper_strategy_factory(
    fve: FairValueEngine, sig: SignalEngine
) -> BaseStrategy:
    return PepperCoreLongStrategy(fve, sig, _V2_CLEAN_CORE_LONG_PARAMS)
...
STRATEGY_REGISTRY = MappingProxyType(
    dict(STRATEGY_REGISTRY, pepper_core_long=_v2_clean_pepper_strategy_factory)
)
'''
...
def _build_wiring_appendix() -> str:
    params_lines = []
    for key, value in V2_CLEAN_PARAMS.items():
...
#                                                                             #
# Phase-10 added this so ``trader_round1_v2_clean.py`` fits under the real    #
...
def _strip_comment_lines(source: str, *, keep_header_lines: int) -> str:
    ...
    comment + V2_clean banner) so the file remains auditable. A line is
...
def _count_header_lines(source: str, *, banner_sentinel: str) -> int:
    """Return the 1-based line number of the V2_clean banner's closing rule.

    The banner ends at the FIRST ``# ====``-style rule that follows
    the ``ROUND-1 UPLOAD VARIANT: v2_clean`` marker (the closing rule
    of the V2_clean banner itself, NOT any subsequent module banner
...
def _minify_bundle(source: str) -> str:
    ...
    banner_sentinel = "ROUND-1 UPLOAD VARIANT: v2_clean"
...
def _build_v2_clean_source() -> str:
    ...
    # 3. Insert V2_clean banner after the auto-generated header. The
    ...
    module_block = [
        ...
        + " (V2_clean: research-only module, inlined at bundle tail)",
        "# " + "=" * 72,
        pepper_body,
    ]
...
def export_v2_clean(*, out_dir: Path = _DEFAULT_OUT_DIR) -> Path:
    out_dir.mkdir(parents=True, exist_ok=True)
    source = _build_v2_clean_source()
...
def main(argv: list[str] | None = None) -> int:
    ...
    path = export_v2_clean(out_dir=args.out_dir)
    size_bytes = path.stat().st_size
    rel = path.relative_to(REPO_ROOT) if path.is_relative_to(REPO_ROOT) else path
    print(f"[v2_clean] wrote {rel} ({size_bytes} bytes)")
    return 0
